...
def latest_catalog():
    ...
    dgf_catalog_df = pd.read_csv(
        dgf_catalog, delimiter=";", compression="zip", error_bad_lines=False
    )
    pd.set_option(
        "display.max_colwidth", None
    )  # to stop pandas from "cutting" long urls
    return dgf_catalog_df


def info_from_catalog(id: str, catalog: pd.DataFrame):
    """
    This function returns a dictionary containing : the resource url, the resource format and the url of its
    data.gouv.fr page -------------------------------------------- :param: id: id of the dgf resource :type: id: string
    """
...
def is_referenced(url, id, catalog_info):
    """
    Given the url of a resource from the catalog, this function returns True if the resource is referenced by
    data.gouv.fr False otherwise :param :url: url of a resource in the catalog :type :url: string
    """
...
def detect_csv(request):
    """
    Given the url request for a csv or txt file, this function returns a dictionary containing the csv encoding and
    separator. ------------------------------------------- :param url: url containing the csv :type url: string
    """
...
def load_dataset(id, catalog_info):
    """
    This function loads a csv in the datasets folder/creates a pandas dataframe given its id if the dataset is
    ...
    if referenced is True:  # if the dataset is referenced
        ...
        if (
            file_format == "csv" or format_is_nan
        ):  # if the format is not available on dgf, we assume it is a csv by default
            if url.rsplit(".", 1)[-1] == "zip":
                ...
                dataframe = pd.read_csv(
                    url, sep=None, engine="python", encoding=encoding
                )
        ...
        dataframe = pd.read_csv(
            Path().home().joinpath(f"open_ML/datasets/resources/{id}/{id}.csv"),
            sep=None,
            engine="python",
        )
        return dataframe
