...
def generate_our_experiments_cards(experiment_path: Path):
            ...
            [
                ...
                dbc.CardBody(
                    [
                        ...
                        html.P("Notre pipeline ML."),
                        dbc.Button(
                            "Voir expérience",
                            color="primary",
                            href=notebook.as_posix(),
                            target="_blank",
                            external_link=True,
                        ),
                    ]
...
def generate_table(dataset_id: str, table_type: str = "dict_data.csv"):
    display_table_path = DATA_PATH.joinpath(f"{dataset_id}/{table_type}")
    if not display_table_path.exists():
        ...
        if "leaderboard" in table_type:
            ...
            table_df["name"] = table_df[["name", "url"]].apply(
                lambda row: html.A(
                    html.P(row["name"]), href=row["url"], target="_blank"
                ),
                axis=1,
            )
...
def generate_mljar_table(dataset_id: str, target_variable: str):
    display_table_path = DATA_PATH.joinpath(
        f"{dataset_id}/automl_{slugify(target_variable)}/leaderboard.csv"
    )
    ...
    if not display_table_path.exists():
        ...
        algorithm_urls = [
            html.A(
                html.P(n),
                href=experiment_path.joinpath(f"{n}/README.html").as_posix(),
                target="_blank",
            )
...
def generate_stats_df(dataset_id: str, table_type: str = "statistics_summary.csv"):
    display_stats_path = DATA_PATH.joinpath(f"{dataset_id}/{table_type}")
...
def generate_dataset_page(dataset_url: str, datasets_df: pd.DataFrame):
    ...
    dictionary_table = generate_table(
        dataset_row["dgf_resource_id"], table_type="dict_data.csv"
    )
    target_vars = list(MLJAR_INFO_DICT[dataset_id].keys())
    statistics_df = generate_stats_df(
        dataset_row["dgf_resource_id"], table_type="statistics_summary.csv"
    )
    pandas_profile_url = DATA_PATH.joinpath(
        f"{dataset_id}/{dataset_id}_pandas_profile.html"
    )
    experiments_url = DATA_PATH.joinpath(f"{dataset_id}/our_experiments/")
    dataset_page_container = dbc.Container(
        [
            html.H5(
                generate_badge(
                    "Accueil",
                    url="/dgml/",
                    background_color="#cadae6",
                    new_tab=False,
                )
            ),
            ...
            html.H4(
                generate_badge(
                    ...
                    url=dataset_row["dgf_dataset_url"],
            ...
            html.H4(
                generate_badge(
                    "Dictionnaire des variables complet",
                    url=dataset_row["dict_url"],
                    background_color="#6d92ad",
                )
            ),
            html.Hr(style={"marginBottom": "20px"}),
            ...
            dbc.CardDeck(
                [
                    generate_kpi_card(
                        "Nombre de colonnes",
                        statistics_df["Number of variables"],
                    ),
                    generate_kpi_card(
                        "Nombre de lignes", statistics_df["Number of lines"]
                    ),
                    generate_kpi_card(
                        "Valeurs manquantes",
                    ...
                    generate_kpi_card(
                        "Variables catégorielles", statistics_df["Categorical"]
                    ),
                    generate_kpi_card(
                        "Haute Cardinalité",
                        statistics_df["High cardinality variables"],
                    ),
                    generate_kpi_card(
                        "Haute Correlation",
                        statistics_df["High correlation variables"],
                    ),
                ]
            ),
            html.H4(
                generate_badge(
                    "Pandas Profile complet",
                    url=pandas_profile_url.as_posix(),
                    background_color="#6d92ad",
                )
            ),
            html.Hr(style={"marginBottom": "20px"}),
            ...
            html.Div(
                [
                    html.P(
                        children=f"Modèles entraînés en utilisant comme target variable : "
                    ),
                    dcc.Dropdown(
            ...
            generate_our_experiments_cards(experiments_url),
            html.Hr(style={"marginBottom": "20px"}),
            # html.H3("Load Data"),
            # html.Hr(style={"marginBottom": "20px"}),
        ]
